...
def room():
    ...
    event = CalendarEvent("event", "Room", now - timedelta(minutes=1), now + timedelta(hours=1),
                          meeting_url="https://teams.microsoft.com/l/meetup-join/fixture")
    calendar = SimpleNamespace(name="calendar", refresh=AsyncMock(), get_event_by_id=lambda _: event)
    info = SimpleNamespace()
    meeting = SimpleNamespace(name="meeting", is_running=True, join_meeting=AsyncMock(return_value=info))
...
@pytest.mark.asyncio
@pytest.mark.parametrize("condition", ["cancelled", "declined", "ended", "future", "missing", "sync_error"])
